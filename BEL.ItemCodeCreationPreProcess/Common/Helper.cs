namespace System.Web.Mvc.Html
{
    ...
    public static class Helper
    {
        ...
        /// </summary>
        private static readonly string[] _validExtensions = { "avi", "pdf", "doc", "docx", "msg", "xls", "xlsx", "csv", "txt", "rtf", "html", "zip", "rar", "mp3", "wma", "mpg", "flv", "jpg", "jpeg", "png", "gif", "ppt", "pptx", "txt", "tif", "tiff", "bmp" };
        ...
        public static string ResourceValue(this HtmlHelper html, string key, ResourceName resourceName)
        {
            if (html != null)
            {
        ...
        public static MvcHtmlString LabelFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, ResourceName resourceName)
        {
        ...
        /// <param name="resourceName">Name of the resource.</param>
        /// <returns></returns>
        public static MvcHtmlString Label(this HtmlHelper html, string expression, ResourceName resourceName)
        {
            if (expression != null && html != null)
            {
                //var memberInfo = Helper.GetMemberInfo(expression);
        ...
        public static MvcHtmlString ValidationMessageFor<TModel, TProperty>(this HtmlHelper<TModel> html, Expression<Func<TModel, TProperty>> expression, ResourceName resourceName)
        {
            if (expression != null && html != null)
            {
                var memberInfo = Helper.GetMemberInfo(expression);
        ...
        /// <param name="resourceName">Name of the resource.</param>
        /// <returns>Span tag with error text</returns>
        public static MvcHtmlString ValidationMessageFor<TModel, TProperty>(this HtmlHelper<TModel> html, Expression<Func<TModel, TProperty>> expression, string resourceKey, ResourceName resourceName)
        {
            if (expression != null && html != null)
            {
        ...
        public static List<ApproverMasterListItem> SplitUser(List<ApproverMasterListItem> items)
        {
            ...
            if (items != null)
            {
                foreach (ApproverMasterListItem item in items)
                {
                    ...
                    else if (item.UserSelection == true && !string.IsNullOrEmpty(item.UserID))
                    {
        ...
        //public static List<TesterMasterListItem> splitAliasUser(List<TesterMasterListItem> items)
        //{
        //    List<TesterMasterListItem> tempDataList = new List<TesterMasterListItem>();
        //    if (items != null)
        //    {
        //        foreach (TesterMasterListItem item in items)
        //        {

        //                string[] strUserName = item.UserName.Split(',');
        //                string[] strUserID = item.UserID.Split(',');
        //                string[] stralias = item.AliasNames.Split(',');
        //                for (int i = 0; i < strUserID.Length; i++)
        //                {
        //                    TesterMasterListItem temprow = new TesterMasterListItem();
        //                    temprow.Title = item.Title;
        //                    temprow.UserName = Convert.ToString(strUserName[i]).Trim();
        //                    temprow.AliasNames = Convert.ToString(stralias[i]).Trim();
        //                    temprow.UserID = strUserID[i];
        ...
        //        }
        //    }
        //    return tempDataList.OrderBy(p => p.AliasNames).ToList();
        //}
        //Security Fixes    
