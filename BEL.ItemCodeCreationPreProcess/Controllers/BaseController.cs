    ...
    public class BaseController : Controller
    {
        ...
        public UserDetails CurrentUser
        {
            get
            {
                ...
                {
                    Logger.Info("sp user == " + this.SPCurrentUser);
                    if (!this.EnvironmentLive)
                    {
                        this.SPCurrentUser = new UserDetails();
                        //this.SPCurrentUser.UserId = "9";
                        //this.SPCurrentUser.UserId = "36"; //cloverspd
                        //this.SPCurrentUser.UserId = "20"; //gadhari suresh
                        //this.SPCurrentUser.UserId = "21"; //collab app2
                        //this.SPCurrentUser.UserId = "22"; //dcrdcnuser3
                         //this.SPCurrentUser.UserId = "6"; //cloverspd1
                          //this.SPCurrentUser.UserId = "23"; //dynadev
                           //this.SPCurrentUser.UserId = "24"; //dipankar
                            //this.SPCurrentUser.UserId = "25"; //collab app1
                             //this.SPCurrentUser.UserId = "26"; //santosh
                              //this.SPCurrentUser.UserId = "27"; //mahadeshwar pravin
                               //this.SPCurrentUser.UserId = "28"; //collab app3
                                this.SPCurrentUser.UserId = "29"; //dcrdcnuser2
                        this.SPCurrentUser = CommonBusinessLayer.Instance.GetUserDetail(Convert.ToInt32(this.SPCurrentUser.UserId));   ///CommonBusinessLayer.Instance.GetLoginUserDetail(this.SPCurrentUser.UserId);                       
                    }
                    else
                    {
                        try
                        {
                            User spUser = null;
                            var spContext = SharePointContextProvider.Current.GetSharePointContext(HttpContext);
                            Logger.Info("clientContext not null -1");
                            using (var clientContext = spContext.CreateUserClientContextForSPHost())
                            {
                                Logger.Info("clientContext not null 0");
                                if (clientContext != null)
                                {
                                    //Logger.Info("clientContext");
                                    //Logger.Info(clientContext);
                                    //clientContext.Load(clientContext.Web);
                                    //Logger.Info("clientContext.Web");
                                    //clientContext.ExecuteQuery();
                                    Logger.Info("clientContext not null");
                                    this.SPCurrentUser = new UserDetails();
                                    spUser = clientContext.Web.CurrentUser;
                                    Logger.Info("clientContext not null 1");
                                    var peopleManager = new PeopleManager(clientContext);
                                    PersonProperties personProperties = peopleManager.GetMyProperties();
                                    Logger.Info("clientContext not null 2");
                                    clientContext.Load(spUser, user => user.Id, user => user.LoginName);
                                    clientContext.Load(personProperties);

                                    Logger.Info("clientContext not null : befor excecuteQuery");

                                    clientContext.ExecuteQuery();

                                    Logger.Info("Contect not null : after excecuteQuery");
        ...
        public JsonResult UploadFileWithStamp(string qqfile)
        {
            ...
            if (!string.IsNullOrEmpty(extension) && (extension.Equals(".jpg") || extension.Equals(".jpeg") || extension.Equals(".png") || extension.Equals(".gif") || extension.Equals(".tif") || extension.Equals(".tiff") || extension.Equals(".bmp")))
            {
                using (MemoryStream ms = new MemoryStream(fileData))
                {
                    ...
                    byte[] imageBytes = this.MergeImages(sourceImg, Server.MapPath(Url.Content("~/images/BajajStamp.png")));
                    ...
                    try
                    {
                        if (imageBytes != null && imageBytes.Length > 0)
                        {
                            ...
                            byte[] output = this.CreateImage(bitmap);
                            fileData = output;
                            bitmap.Dispose();
                        }
                    }
        ...
        /// Creates the image.
        /// </summary>
        /// <param name="bitMapImage">The bit map image.</param>
        /// <returns></returns>
        ...
        /// <param name="stampImagePath">The stamp image path.</param>
        /// <returns></returns>
        ...
        /// message lsit
        /// </returns>
        protected List<string> GetErrorMessage(ResourceName resourceFile)
        {
        ...
        public ActionStatus GetMessage(ActionStatus status, ResourceName resourFilName)
        {
            if (status != null)
            {
        ...
        public string GetResourceValue(string key, ResourceName resourFilName)
        {
        ...
        /// <summary>
        /// Adds the list records.
        /// </summary>
        /// <typeparam name="T">The T</typeparam>
        /// <param name="lstmodel">The lstmodel.</param>
        /// <param name="columnName">Name of the column.</param>
        /// <param name="i">The i.</param>
        /// <returns>returns T</returns>
        protected T AddListRecords<T>(List<T> lstmodel, string columnName, int i = 0) where T : new()
        {
            T t = new T();
            t.GetType().GetProperty(columnName.ToString()).SetValue(t, i, null);
            lstmodel.Add(t);
            return t;
        }
    }
