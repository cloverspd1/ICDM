...
jQuery(document).ready(function () {
    ...
    jQuery(document).on("click", ".parentChks", function (e) {
        if (this.checked == false) {
            $('#bs-collapse > .panel').each(function () {
                if (searchTerm != '' && searchTerm != undefined) {
                    panelContainerId = '#' + $(this).attr('id');
                    $(panelContainerId + ':containsCaseInsensitive(' + searchTerm + ')').find('input:checkbox[save="save"]').prop('checked', false);
                }
                else {
                    jQuery(document).find('input:checkbox[save="save"]').prop('checked', false);
                }
            });
        }
            ...
            $('#bs-collapse > .panel').each(function () {
                if (searchTerm != '' && searchTerm != undefined) {
                    panelContainerId = '#' + $(this).attr('id');
                    $(panelContainerId + ':containsCaseInsensitive(' + searchTerm + ')').find('input:checkbox[save="save"]').prop('checked', true);
                }
                else {
                    jQuery(document).find('input:checkbox[save="save"]').prop('checked', true);
                }
            });
        }
    });
    ...
    jQuery(document).on("click", ".parentChk", function (e) {
        ...
        if (jQuery(this).closest("input:checkbox").prop('checked')) {
            jQuery("input:checkbox[data='" + dataAttr + "C']").prop('checked', true);
        }
        else {
            jQuery("input:checkbox[data='" + dataAttr + "C']").prop('checked', false);
        }
        checkAllParetntCheckbox();
    });
    jQuery(document).on("click", ".childCheck", function (e) {
        ...
        jQuery("input:checkbox[data='" + dataAttr + "C']").each(function () {
            if (this.checked == false) {
                allChecked = false;
                jQuery("input:checkbox[data='" + dataAttr + "']").prop('checked', false);
            }
        })
        if (allChecked == true) {
            jQuery("input:checkbox[data='" + dataAttr + "']").prop('checked', true);
        }
        checkAllParetntCheckbox();
    });
    ...
    jQuery('#btnAddRole').on('click', function () {
        ...
        jQuery('#lblRoleError').addClass('hidden');
        if (jQuery('#btnAddRole').val() == "Save Role") {
            ...
            if (roleName != "") {
                ...
                jQuery('#lblRoleError').removeClass('hidden');
            }
    ...
    jQuery('#btnDeleteRole').on('click', function () {
        ...
        if (Id > 0) {
            ...
            if (roleName == 'admin' || roleName == 'iccp admin' || roleName == 'lum marketing incharge' || roleName == 'lum marketing delegate' || roleName == 'lum design incharge' ||
                roleName == 'lum design delegate' || roleName == 'sms incharge' || roleName == 'sms delegate' || roleName == 'qa incharge' || roleName == 'qa delegate' ||
                roleName == 'final sms incharge' || roleName == 'final sms delegate' || roleName == 'costing incharge' || roleName == 'costing delegate1' || roleName == 'costing delegate2' ||
                roleName == 'tds incharge' || roleName == 'tds delegate') {
...
function checkAllParetntCheckbox() {
    ...
    jQuery(document).find("input:checkbox[class='parentChk']").each(function () {
        if (this.checked == false) {
            allChecked = false;
            jQuery(document).find(".parentChks").prop('checked', false);
        }
    });
    if (allChecked == true) {
        jQuery(document).find(".parentChks").prop('checked', true);
    }
}
            ...
            title: "Confirm", message: attachmsg, okCallback: function (data) {
                ...
                jQuery(document).find('input:checkbox[save="save"]').each(function (i, e) {
                    ...
                    var IdValueData = {
                        ...
                        IsAuthorized: (jQuery(e).prop('checked')) ? true : false,
                        RoleName: roleName
                    };
