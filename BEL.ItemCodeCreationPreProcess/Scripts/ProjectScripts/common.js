...
function BindValidation() {
    ...
    $(".alphanumeric").off("keypress").on("keypress", function (event) {
    ...
    $(".amount").off("change").on("change", function () {
        var txtAmount = $(this).val();
        if (txtAmount == '') {
            txtAmount = $(this).text();
        }
        $(this).text(txtAmount.replace(/,/g, ''));
        $(this).val(txtAmount.replace(/,/g, ''));
    });
}
...
function DisableFormItems() {
    ...
    if ($('.token-input-dropdown').length > 0) {
...
function BindUserTags(selector) {
    ...
    $(selector + ".user-tags").each(function () {
        ...
        var prepoulated = $(ele).data('prepopulateitems');

        var disabledTag = $(ele).attr('data-disabledItem') == "True" ? true : false;;
        ...
        if (typeof (prepoulated) === "undefined" || prepoulated == null || prepoulated == '') {
            prepoulated = null;
        }
        if (typeof (disabledTag) === "undefined" || disabledTag == null || disabledTag == '') {
            disabledTag = false;
        }
...
function BindUserTagsForGrid(selector) {
    $(selector + ".user-tags").each(function () {
        var ele = $(this);
        var valueList = ele.val().split(',');
        var url = BASEPATHURL + ele.attr("data-url");
            ...
            onAdd: function (item) {
                if (typeof (onadd) !== "undefined" && onadd != null && onadd != '') {
                    if (item.items != undefined) {
                        eval(onadd + "('#" + ele.attr("id") + "','" + item.id + "','" + encodeURIComponent(item.name) + "','" + JSON.stringify(item.items) + "');");
                    }
                    else {
                        eval(onadd + "('#" + ele.attr("id") + "','" + item.id + "','" + encodeURIComponent(item.name) + "');");
                    }
...
function BindFileUploadControlForGrid(options) {
    /*
...
function ConfirmSubmit(ele) {
    ...
    formList.each(function () {
        ...
        if ($(this).attr('id') == "frmSCMLUMDesignInchrage" && dataAction == 10) {
            isValid = ValidateVendor($(this));
        }

        if ($(this).attr('id') == "frmSCMLUMDesignDelegate" && dataAction != 33)
            isValid = ValidateVendor($(this));
...
function ValidateVendor(obj) {
    var isValid = true;
    $(obj).find('ul.token-input-list').each(function (i, e) {
        if ($(e).find('li.token-input-token').length == 0) {
            isValid = false;
            $(e).addClass('input-validation-error');
            var validationmesage = $(e).next().attr('data-val-required');
            $(e).next().next().removeClass('field-validation-valid').addClass('field-validation-error');
            $(e).next().next().attr('data-valmsg-replace', 'false');
            $(e).next().next().html(validationmesage);
        }
    });
...
function PrintWithAttachment(ele) {
    ...
    $.ajax({
        url: BASEPATHURL + "ItemCodeCreation/GetPdf",
        type: "POST",
...
function isInteger(evt, control) {
    ...
    //Char Code : 46 for "."
    //Allow only Numbers
    if ((charCode == 46 && control.value.indexOf('.') != -1) || (charCode < 48 || charCode > 57)) {
        return false;
    }
    return true;
}
...
function ValidateUser(eleId, emailId, ItemName) {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (eleId != elements[i][0]) {
                if ($(elements[i][0]).val().indexOf(trimdEmail) != -1) {
                    ...
                    valid = false;
                    AlertModal(ResourceManager.Title.Error, ItemName.substring(0, ItemName.indexOf(" -")) + " is already entered as " + elements[i][1] + " role, so you can't make same user as " + currentFieldRole + " in this request.", false, function () {
                        setTimeout(function () {
    ...
    if (valid && eleId == "#Convener") {
        ...
        if ((requestBy.indexOf(trimdEmail) != -1 && onBehalfOf != '') && convener != '') {
            $(eleId).tokenInput("remove", { id: emailId });
            AlertModal(ResourceManager.Title.Error, "Creator cannot be convener as On Behalf Of is selected.", false, function () {
                setTimeout(function () {
                    $(eleId).parent().find("input:first").focus();
                }, 100);
            });
...
function ValidateGridRecords(TableObjectId, IgnoreBlankValues) {
    if (IgnoreBlankValues == undefined)
        IgnoreBlankValues = true;
    jQuery('#' + TableObjectId + ' input:text, #' + TableObjectId + ' select, #' + TableObjectId + ' textarea').removeClass('input-validation-error');
    var noerror = true;
    jQuery('#' + TableObjectId + ' > tbody > tr').each(function (i, e) {
        var totalElement = 0;
        var blanckValueCount = 0;
        totalElement = jQuery('td input:text,select,textarea', e).length;
        jQuery('td input:text,select,textarea', e).each(function (index, control) {
        ...
        if (jQuery(e).is(':visible') && ((totalElement != blanckValueCount && IgnoreBlankValues) || !IgnoreBlankValues)) {
            jQuery('td input:text,select,textarea', e).each(function (index, control) {
                ...
                if (jQuery(control).attr('datarequired') != undefined) {
