namespace BEL.DataAccessLayer
{
    ...
    public class AppApprovalMatrixHelper : BELDataAccessLayer
    {
        ...
        public List<SLAMatrix> GetSLAMatrix(string applicationName, string formName)
        {
            ...
            string siteURL = Instance.GetConfigVariable("RootSiteURL");
            ClientContext context = Instance.CreateClientContext(siteURL);
            Web web = Instance.CreateWeb(context);
            context.Load(web);
        ...
        public ApplicationStatus GetGlobalAppApproverMatrixForCurrentRole(string applicationName, string formName, string role)
        {
            ...
            ClientContext context = CreateClientContext(siteURL);
            Web web = CreateWeb(context);
            context.Load(web);
            context.ExecuteQuery();
        ...
        public List<ApplicationStatus> GetAppApprovalMatrix(ClientContext context, Web web, int itemId, string approvalMatrixListName, bool getAllRecords = false)
        {
            ...
            {
                approvalMatrix = Instance.GetConfigVariable("UseRESTAPI").ToLower().Equals("true")
                    ? this.GetAppApprovalMatrixUsingREST(context, web, itemId, approvalMatrixListName)
                    : this.GetAppApprovalMatrixUsingCSOM(context, web, itemId, approvalMatrixListName, getAllRecords);
            }
                ...
                if (isListColumn)
                {
                    ...
                    else if (isFile)
                    {
                        if (((dynamic)((dynamic)item["AttachmentFiles"])).__deferred != null)
                        {
                            ...
                            List<FileDetails> objAttachmentFiles = Instance.GetAttachmentsUsingREST(url);
        ...
        public ApplicationStatus GetAppApprovalMatrixByRole(ClientContext context, Web web, string role, int itemId, string approverMatrixListName, string userEmail)
        {
            ...
            {
                User user = EnsureUser(context, web, userEmail);
        ...
        public bool SetAppApprovalMatrixByRole(ClientContext context, Web web, string role, int itemId, string approverMatrixListName, string userEmail, string newUserEmailToAppend)
        {
            if (context != null && web != null && !string.IsNullOrEmpty(approverMatrixListName) && itemId > 0)
            {
                ...
                if (item != null)
                {
                    string approverEmails = BELDataAccessLayer.GetUserIDsFromPersonField(context, web, item["Approver"] as FieldUserValue[]).Trim(',') + "," + newUserEmailToAppend;
            ...
            foreach (PropertyInfo property in propertyInfo)
            {
                ...
                if (isListColumn)
                {
                    if (property.GetCustomAttribute<FieldColumnNameAttribute>() != null && property.GetCustomAttribute<FieldColumnNameAttribute>().IsLookup)
                    {
                        ...
                        if (lookupField != null) property.SetValue(approver, lookupField.LookupId);
                    }
                    else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnID)
                    {
                        ...
                        if (users != null)
                        {
                            string userIDs = GetUserIDsFromPersonField(context, web, users);
                            property.SetValue(approver, userIDs);
                        }
                    }
                    else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                    {
                        ...
                        if (users != null)
                        {
                            string personEmails = BELDataAccessLayer.GetPersonValueFromPersonField(context, web, users, Person.Name);
                    ...
                    else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnAlias)
                    {
                        ...
                        if (users != null)
                        {
                            string personAliases = GetPersonValueFromPersonField(context, web, users, Person.Alias);
                            property.SetValue(approver, personAliases);
                        }
                    }
                    else if (isFile)
                    {
                        if (Convert.ToString(item["Attachments"]) == "True")
                        {
                            ...
                            List<FileDetails> objAttachmentFiles = Instance.GetAttachments(attachments);
                            property.SetValue(approver, objAttachmentFiles);
                        }
                    }
                    else
        ...
        private List<ApplicationStatus> GetAllGlobalAppApproverMatrix()
        {
            ...
            foreach (ListItem item in items)
            {
                approverMatrixList.Add(new ApplicationStatus()
                {
                    ...
                    Approver = GetUserIDsFromPersonField(context, web, item["Approver"] as FieldUserValue[]),
                    /* Approver Copy from Global Matrix*/
                    Division = Convert.ToString(item["Division"]),
                    Days = Convert.ToInt32(item["Days"]),
                    IsAutoApproval = (bool)item["IsAutoApproval"],
        ...
        public bool SaveApproverMatrix(ClientContext context, Web web, List<ApplicationStatus> approverMatrixList, string listName) // Jatin 
        {
            if (context != null && web != null && approverMatrixList != null)
            {
                ...
                var approverItems = new ListItem[approverMatrixList.Count];
                int i = 0;
                foreach (ApplicationStatus approver in approverMatrixList)
                {
                    if (approver.ID > 0)
                    {
                        ...
                        approverItems[i].RefreshLoad();//Version Conflict
                    ...
                    foreach (PropertyInfo property in itemProperties)
                    {
                        ...
                        bool isReturnId = property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().ReturnID;
                        ...
                        //bool isReturnAlias = property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().ReturnAlias;
                        string listCoumnName = property.GetCustomAttribute<FieldColumnNameAttribute>() != null && !string.IsNullOrEmpty(property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation) ? property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation : property.Name;
                        if (isListCoumn)
                        {
                            if (isPerson)
                            {
                                if (!isReturnName && isReturnId)
                                {
        ...
        public List<ApplicationStatus> UpdateStatusofApprovalMatrix(List<ApplicationStatus> approvers, int currLevel, int preLevel, Dictionary<string, string> param, ButtonActionStatus actionPerformed)
        {
            if (approvers != null && param != null)
            {
                if (currLevel != preLevel)
                {
                    ...
                    if (approvers.Count != 0 && !string.IsNullOrEmpty(userEmail))
                    {
                        ...
                        switch (actionPerformed)
                        {
                            ...
                            case ButtonActionStatus.None:
                                Logger.Info("Save as draft condition => any approver=" + approvers.Any(p => p.Levels == currLevel.ToString()));
                                if (approvers.Any(p => p.Levels == currLevel.ToString()))
                                {
                                    approvers.ForEach(p =>
                                    {
                                        ...
                                        if (p.Levels == currLevel.ToString() && p.Status == ApproverStatus.NOTASSIGNED)
                                        {
                                            ...
                                            p.DueDate = GetDueDate(DateTime.Now, Convert.ToInt32(p.Days));
                            ...
                            case ButtonActionStatus.Hold:
                                ...
                                if (approvers.Any(p => p.Levels == currLevel.ToString()))
                                {
                                    approvers.ForEach(p =>
                                    {
                                        Logger.Info(p.Levels + "==" + currLevel.ToString() + " && " + p.Status + "==" + ApproverStatus.PENDING);
                                        if (p.Levels == currLevel.ToString() && p.Status == ApproverStatus.PENDING)
                                        {
                                            Logger.Info("condition true for => " + JsonConvert.SerializeObject(p));
                                            p.Status = ApproverStatus.HOLD;
                                        }
                                    });
                                }
                                break;
                            case ButtonActionStatus.Resume:
                                ...
                                if (approvers.Any(p => p.Levels == currLevel.ToString()))
                                {
                                    approvers.ForEach(p =>
                                    {
                                        Logger.Info(p.Levels + "==" + currLevel.ToString() + " && " + p.Status + "==" + ApproverStatus.HOLD);
                                        if (p.Levels == currLevel.ToString() && p.Status == ApproverStatus.HOLD)
                                        {
                                            Logger.Info("condition true for => " + JsonConvert.SerializeObject(p));
                                            p.Status = ApproverStatus.PENDING;
                            ...
                            case ButtonActionStatus.NextApproval:
                                ...
                                {
                                    approvers.FirstOrDefault(p =>
                                        !string.IsNullOrEmpty(p.Approver) && p.Levels == currLevel.ToString() &&
                                        p.Approver.Contains(userEmail)).Status = ApproverStatus.APPROVED;
                                }
                                ...
                                approvers.ForEach(p =>
                                {
                                    if (!string.IsNullOrEmpty(p.Approver) && p.Levels.Equals(currLevel.ToString()) && (p.Approver.IndexOf(',') > 0 ? p.Approver.Split(',').Contains(userEmail) : p.Approver == userEmail))
                                    {
        ...
        {
            string strHolidays = GetConfigVariable("HolidaysList");
            ...
            if (approvers != null && approvers.Count != 0)
            {
                ...
                approvers.ForEach(p =>
                {
                    if (!string.IsNullOrEmpty(p.Approver))
                    {
                        ...
                        {
                            if (p.Levels == curLevel.ToString() && (p.Status == ApproverStatus.PENDING || p.Status == ApproverStatus.HOLD || p.Status == ApproverStatus.RESUME))
                            {
                                /* All users 
                            ...
                            ////Phase 2 :All members who will be in the ItemCodeCreation Process should be able to know the status of all ICCP Request. 
        ...
        public string GetAppApprovalMatrixUsersEmail(ClientContext context, Web web, int requestId, string approverListName, string fromLevel = "", string toLevel = "")
        {
            ...
            if (context != null && web != null && requestId > 0 && !string.IsNullOrEmpty(approverListName))
            {
                ...
                if (items.Count > 0)
                {
                    foreach (ListItem item in items)
                    {
                        userEmails = userEmails.Trim(',') + "," + BELDataAccessLayer.GetUserIDsFromPersonField(context, web, item["Approvers"] as FieldUserValue[]);
                    }
                }
            }
