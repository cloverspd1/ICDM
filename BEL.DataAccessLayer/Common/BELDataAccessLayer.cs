namespace BEL.DataAccessLayer
{
    ...
    public class BELDataAccessLayer
    {
        ...
        public static string GenerateReferenceNo(string appName, string itemId, DateTime requestDate)
        {
            ...
            switch (appName)
            {
                case ApplicationNameConstants.LUMITEMCODECREATION:
                    appName = "LUMITEMCODECREATIONAPP";
                    break;
            }
        ...
        /// string value
        /// </returns>
        public static string GetUserIDsFromPersonField(ClientContext context, Web web, FieldUserValue user)
        {
            string userEmail;
            BELDataAccessLayer helper = new BELDataAccessLayer();
            if (helper.GetConfigVariable("UseRESTAPI").ToLower().Equals("true"))
            {
                userEmail = GetUserIDsFromPersonFieldUsingREST(context, web, user);
            }
            else
            {
                userEmail = GetUserIDsFromPersonFieldUsingCSOM(context, web, user);
            }
            return userEmail;
        }
        ...
        public static string GetUserIDsFromPersonFieldUsingREST(ClientContext context, Web web, FieldUserValue user)
        {
        ...
        /// string value
        /// </returns>
        public static string GetUserIDsFromPersonFieldUsingCSOM(ClientContext context, Web web, FieldUserValue user)
        {
        ...
        /// <summary>
        /// Gets the user i ds from person field.
        /// </summary>
        /// <param name="context">The context.</param>
        /// <param name="web">The web.</param>
        /// <param name="users">The users.</param>
        /// <returns></returns>
        public static string GetUserIDsFromPersonField(ClientContext context, Web web, FieldUserValue[] users)
        {
            string userId = string.Empty;
            if (users != null && users.Length > 0 && web != null && context != null)
            {
                ...
                {
                    string userEmail = GetUserIDsFromPersonField(context, web, user);
        ...
        /// Get person names
        /// </returns>
        public static string GetPersonValueFromPersonField(ClientContext context, Web web, FieldUserValue[] users, Person returnValue)
        {
            string persons = string.Empty;
            var employees = new List<EmployeeDetails>();
            var masterHelper = new MasterDataHelper();
            if (users != null && users.Length > 0 && web != null && context != null)
            {
                foreach (var user in users)
                {
                    Logger.Info("Start Helper.GetAlias", user.LookupId);
                    switch (returnValue)
                    {
                        case Person.Name:
                            persons += !string.IsNullOrEmpty(user.LookupValue) ? "," + user.LookupValue : string.Empty;
                            break;
                        case Person.Alias:
                            string emailId;
                            try
                            {
                                emailId = ((dynamic)user).Email;
                            }
                            catch (Exception e)
                            {
                                var varUser = EnsureUser(context, web, Convert.ToString(user.LookupId));
                                emailId = varUser.Email;
                                Console.WriteLine(e);
                                Logger.Info("User resolved : ", emailId);
                            }
                            ...
                            var alise = masterHelper.GetEmployeeAliasByEmail(context, web, emailId);
                            persons = persons + (!string.IsNullOrEmpty(alise) ? "," + alise : string.Empty);
                            Logger.Info("End Helper.GetAlias", user.LookupId);
                            break;
                    }
                }
            }

            persons = persons.Trim(',').Trim();
            return persons;
        }
        ...
        /// Get person names
        /// </returns>
        public static List<EmployeeTag> GetEmployeeTagsFromPersonField(ClientContext context, Web web, FieldUserValue[] users)
        {
            ...
            if (users != null && users.Length > 0 && web != null && context != null)
            {
                foreach (var user in users)
                {
                    Logger.Info("Start Helper.Get tag", user.LookupId);
                    ...
                    var emp = masterHelper.GetEmployeeByEmail(context, web, emailId);
                    if (emp != null)
                        employeetags.Add(new EmployeeTag
                        {
                            Id = emp.UserID,
                            Name = emp.Alias
                        });

                    Logger.Info("End Helper.Get tag", user.LookupId);
                }
            }
            return employeetags;
        }
        ...
        /// Get person names
        /// </returns>
        public static string GetPersonValueFromPersonField(ClientContext context, Web web, FieldUserValue user, Person returnValue)
        {
            string persons = string.Empty;
            var masterHelper = new MasterDataHelper();
            if (user != null && web != null && context != null)
            {
                switch (returnValue)
                {
                    ...
                    case Person.Alias:
                        ...
                        var alias = masterHelper.GetEmployeeAliasByEmail(context, web, emailId);
                        persons = !string.IsNullOrEmpty(alias) ? "," + alias : string.Empty;
                        Logger.Info("End Helper.GetAlias", user.LookupId);
                        break;
                }
            }
            return persons.Trim(',').Trim();
        }
        ...
        public static string GetNameFromPersonField(ClientContext context, Web web, FieldUserValue user)
        {
            ...
            {
                BELDataAccessLayer helper = new BELDataAccessLayer();
                personsName = helper.GetConfigVariable("UseRESTAPI").ToLower().Equals("true") ? GetNameFromPersonFieldUsingREST(context, web, user) : GetNameFromPersonFieldUsingCSOM(context, web, user);
            }
            return personsName;
        }
        ...
        public static FieldUserValue GetFieldUserValueFromPerson(ClientContext context, Web web, string userID)
        {
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(userID))
            {
                ...
                objuser = new FieldUserValue { LookupId = Convert.ToInt32(userID) };
            }
            return objuser;
        }
        ...
        /// <returns>
        /// FielduserValue Object
        /// </returns>
        public static FieldUserValue[] GetAliaseFromUserID(ClientContext context, Web web, string userIDs)
        {
            var ides = userIDs.Split(',');
            var lstFieldUserValues = new List<FieldUserValue>();
            foreach (var id in ides)
            {
                var varFiledUser = GetFieldUserValueFromPerson(context, web, id);
                if (varFiledUser != null) lstFieldUserValues.Add(varFiledUser);
            }

            var lstFieldUserValuesarrayValues = lstFieldUserValues.ToArray();

            return lstFieldUserValuesarrayValues;
        }
        ...
        /// <param name="userIds">The emails.</param>
        /// <returns>FielduserValue[] Object</returns>
        public static FieldUserValue[] GetFieldUserValueFromUserIds(ClientContext context, Web web, string[] userIds)
        {
            FieldUserValue[] users = null;
            if (userIds != null && userIds.Length > 0)
            {
                users = new FieldUserValue[userIds.Length];
                int i = 0;
                foreach (string email in userIds)
                {
        ...
        /// <returns></returns>
        public static string GetReceiverName(string userNames)
        {
            string receiverName = string.Empty;
            if (!string.IsNullOrWhiteSpace(userNames) && userNames.IndexOf(',') >= 0)
            {
                string[] multipleUserName = userNames.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                if (multipleUserName != null)
                {
                    if (multipleUserName.Length > 1)
                    {
                        receiverName = "Everyone";
                    }
                    else if (multipleUserName.Length == 1)
                    {
                        receiverName = userNames;
                    }
                }
            }
            else if (!string.IsNullOrWhiteSpace(userNames) && userNames.IndexOf(',') < 0)
            {
                receiverName = userNames;
            }
            return receiverName;
        ...
        public ClientContext CreateClientContext(string strURL)
        {
            ...
            return context;
            //string realm = TokenHelper.GetRealmFromTargetUrl(new Uri(strURL));
            //Logger.Info("realm " + realm);
            //string accessToken = TokenHelper.GetAppOnlyAccessToken(TokenHelper.SharePointPrincipal, new Uri(strURL).Authority, realm).AccessToken;
            //Logger.Info("accessToken " + accessToken);
            //using (context = TokenHelper.GetClientContextWithAccessToken(strURL, accessToken))
            //{
            //    Logger.Info("test context not null");
            //    return context;
            //}
        }
        ...
        /// return clientcontext object
        /// </returns>
        public ClientContext CreateClientContextforattachment(string strURL)
        {
            ClientContext context = null;
            using (context = new ClientContext(strURL))
            {
        ...
        //public ClientContext CreateRootContext(string strURL)
        //{
        //    ClientContext context = null;
        //    using (context = new ClientContext(strURL))
        //    {
        ...
        /// <param name="isTranRequestIdUpdate">if set to <c>true</c> [is tran request identifier update].</param>
        /// <returns>
        /// list of listitem details
        /// </returns>
        public List<ListItemDetail> SaveData(ClientContext context, Web web, ISection section, Dictionary<string, string> param, Dictionary<string, string> mailCustomValues = null, List<FileDetails> emailAttachments = null, bool isTranRequestIdUpdate = true)
        {
                ...
                if (listCount > 0)
                {
                    foreach (ListItemDetail listDetail in actualSection.ListDetails)
                    {
                        try
                        {
                            ...
                            Logger.Info("isNewItem=" + isNewItem);
                            objListItem = new ListItemDetail();
                            if (param != null)
                            {
                                param[Parameter.ISNEWITEM] = isNewItem.ToString();
                            }
                            Logger.Info("Save data context=" + context);
                            Logger.Info("Save data web=" + web);
                            Logger.Info("actualSection=" + actualSection);
                            Logger.Info("item=" + item);
                            Logger.Info("listDetail.ListName=" + listDetail.ListName);
                            Logger.Info("param=" + param);
                            Logger.Info("isTranRequestIdUpdate=" + isTranRequestIdUpdate);
                            int itemId = this.SaveDataInList(context, web, actualSection, item, listDetail.ListName, param, mailCustomValues, emailAttachments, isTranRequestIdUpdate);
                ...
                foreach (FileDetails file in files)
                {
                    try
                    {
                        ...
                        else if (file.Status == FileStatus.New)
                        {
                            ...
                            AttachmentCreationInformation aci =
        ...
        /// int value
        /// </returns>
        public int SaveDataInList(ClientContext context, Web web, dynamic actualSection, ListItem item, string listName, Dictionary<string, string> param, Dictionary<string, string> mailCustomValues = null, List<FileDetails> emailAttachments = null, bool isTranRequestIdUpdate = true)
        {
            int itemId = 0;
            ...
            if (context != null && web != null && param != null && item != null)
            {
                ...
                foreach (PropertyInfo property in properties)
                {
                    ...
                    if (isListCoumn && isCurrentListCoumn && !notSaved)
                    {
                        ...
                        else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnID)
                        {
                        ...
                        {
                            // itemDictionary[propertyName + "Name"] = property.GetValue(actualSection);
                        }
                        ...
                        {
                            //itemDictionary[propertyName + "Alias"] = property.GetValue(actualSection);
                        }
                        else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().EmployeeTags)
                        {
                            //itemDictionary[propertyName + "Alias"] = property.GetValue(actualSection);
                        }
                        else
                    ...
                    if (property.GetCustomAttribute<IsViewerAttribute>() != null && property.GetCustomAttribute<IsViewerAttribute>().IsViewer && property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().ReturnID)
                    {
                ...
                if (isNewItem)
                {
                    ...
                    {
                        formFieldValues.Add("ReferenceNo", GenerateReferenceNo(param[Parameter.FORMNAME], item["ID"].ToString(), Convert.ToDateTime(item["RequestDate"])));
                ...
                {
                    Logger.Info("Saving Local Approver Matrix '" + approverMatrixListName + "'" + "List=" + JsonConvert.SerializeObject(approversDataFromList));
                    if (isNewItem)
                    {
                        approversDataFromList = approverHelper.GetGlobalAppApproverMatrix(param[Parameter.APPLICATIONNAME], param[Parameter.FORMNAME]);
                ...
                {
                    AsyncHelper.Call(obj => { this.SaveAllTransOfSection(context, web, transList, transListName, approvalMatrixUsers, itemId, param, isTranRequestIdUpdate); });
                }
                Logger.Info("Started form level assignment");
                ButtonActionStatus actionPerformed = actualSection.ActionStatus;
                if (actionPerformed == ButtonActionStatus.RestartToUpdate)
                {
                    currLevel = 0;
                }
                if (!string.IsNullOrEmpty(approverMatrixListName))
                {
                    approversDataFromList.ForEach(p =>
                    {
                        if (!string.IsNullOrEmpty(p.Role) && !string.IsNullOrEmpty(p.Approver))
                        {
                            ...
                            formFieldValues[userRole.Substring(0, userRole.Length > 32 ? 32 : userRole.Length)] = GetFieldUserValueFromUserIds(context, web, p.Approver.Trim(',').Split(','));
                        }
                    });
                    ...
                    if (!approversDataFromList.Any(p => p.IsOptional == false && !string.IsNullOrEmpty(p.Approver) && p.Status != ApproverStatus.APPROVED && Convert.ToInt32(p.Levels) == currLevel)
                        && actionPerformed != ButtonActionStatus.SendBack && actionPerformed != ButtonActionStatus.RestartToUpdate && actionPerformed != ButtonActionStatus.Forward)
                    {
                    ...
                    else
                    {
                        if (actionPerformed == ButtonActionStatus.NextApproval || actionPerformed == ButtonActionStatus.Delegate)
                        {
                            ...
                            //// 7 Aug 2018 Pooja : When immediate delegate(when incharge and delegate both are on same level) applied in LAST LEVEL of workflow.
                            if (approver == null && actionPerformed == ButtonActionStatus.Delegate)
                            {
                                approver = approversDataFromList.Where(p => Convert.ToInt32(p.Levels) >= currLevel && !string.IsNullOrEmpty(p.Approver) && p.Status != ApproverStatus.NOTREQUIRED).OrderBy(p => Convert.ToInt32(p.Levels)).FirstOrDefault();
                            }

                            if (approver != null)
                            {
                                var listofNextApprovers = approversDataFromList.Where(p => Convert.ToInt32(p.Levels) == nextLevel && p.Status == ApproverStatus.PENDING).ToList();
                                listofNextApprovers.ForEach(p =>
                                {
                                    if (!string.IsNullOrEmpty(p.Approver))
                                    {
                    ...
                    if (actionPerformed == ButtonActionStatus.SendBack && param.ContainsKey(Parameter.SENDTOLEVEL) && !string.IsNullOrEmpty(param[Parameter.SENDTOLEVEL]))
                    {
                    ...
                    ////Handel Send Back or back to Creator Case.    added restart to update case to make request available with creator
                    if (actionPerformed == ButtonActionStatus.RestartToUpdate)
                    {
                        nextLevel = Convert.ToInt32(param[Parameter.SENDTOLEVEL]);
                        var listofNextApprovers = approversDataFromList.Where(p => Convert.ToInt32(p.Levels) == nextLevel).ToList();
                        nextApprover = string.Empty; // 30/03/2019 Jatin: to reset nextapprover field
                        listofNextApprovers.ForEach(p =>
                        {
                            if (!string.IsNullOrEmpty(p.Approver))
...
#pragma warning disable CS0219 // The variable 'isTaskAssignMailSend' is assigned but its value is never used
                bool isTaskAssignMailSend = false;
#pragma warning restore CS0219 // The variable 'isTaskAssignMailSend' is assigned but its value is never used
                switch (actionPerformed)
                {
                    case ButtonActionStatus.SaveAndNoStatusUpdate:
                        formFieldValues.Add("NextApprover", GetFieldUserValueFromUserIds(context, web, currentUserID.Trim(',').Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries)));
                        break;
                    ...
                    case ButtonActionStatus.Hold:
                        formFieldValues.Add("Status", FormStatus.HOLD);
                        formFieldValues.Add("HoldDate", DateTime.Today);
                        ...
                        formFieldValues.Add("PendingWith", currentUserRole);
                        break;
                    ...
                    case ButtonActionStatus.NextApproval:
                        ...
                        if (!string.IsNullOrEmpty(nextApprover))
                        {
                            FieldUserValue[] nextApproverFieldValue = GetFieldUserValueFromUserIds(context, web, nextApprover.Trim(',').Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries));
                    ...
                    case ButtonActionStatus.RestartToUpdate:
                        ...
                        if (!string.IsNullOrEmpty(nextApprover))
                        {
                            var nextApproverFieldValue = GetFieldUserValueFromUserIds(context, web, nextApprover.Trim(',').Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries));
                            formFieldValues.Add("NextApprover", nextApproverFieldValue);
                        }
                ...
                // Set Aliases of the Next Approver.. 16-mar-18 KK Soni
                //if (formFieldValues.ContainsKey("NextApprover"))
                //{
                //if (actionPerformed == ButtonActionStatus.SaveAndNoStatusUpdate)
                //{
                //    FieldUserValue[] userValues = formFieldValues["NextApprover"];
                //    string usersAliasFromPersonField = GetPersonValueFromPersonField(context, web, userValues, Person.Alias);
                //    formFieldValues.Add("NextApproverAlias", usersAliasFromPersonField);
                //}
                //else if (formFieldValues["Status"] != "Completed" && formFieldValues["Status"] != "Rejected")
                //{
                //    FieldUserValue[] userValues = formFieldValues["NextApprover"];
                //    string usersAliasFromPersonField = GetPersonValueFromPersonField(context, web, userValues, Person.Alias);
                //    formFieldValues.Add("NextApproverAlias", usersAliasFromPersonField);
                ...
                //else if (formFieldValues["Status"] == "Completed" || formFieldValues["Status"] == "Rejected")
                //{
                //    formFieldValues.Add("NextApproverAlias", string.Empty);
                //}
                //}
                ...
                AsyncHelper.Call(obj =>
                {
                    this.SendMail(actionPerformed, context, web, Convert.ToString(param[Parameter.USEREID]), itemId, approversDataFromList, listName, nextLevel, currLevel, currentUserRole, param, mailCustomValues, emailAttachments);
                });
                /*Send Email End*/
            }
            return itemId;
        ...
        /// true or false
        /// </returns>
        private bool SaveAllTransOfSection(ClientContext context, Web web, List<List<ITrans>> transList, List<string> transListName, string approvalMatrixUsers, int itemId, Dictionary<string, string> objparam, bool isTranRequestIdUpdate = true)
        {
            bool isSuccess = true;
            for (int i = 0; i < transList.Count && isSuccess; i++)
            {
                if (isTranRequestIdUpdate)
                {
            ...
            if (formFieldValues.ContainsKey("Status"))
            {
                ...
                switch (status)
                {
                    ...
                    case FormStatus.SENTBACK:
                        ...
                        break;
                    case FormStatus.HOLD:
                        workflowStatus = "Hold by " + (formFieldValues.ContainsKey("LastActionByRole") ? Convert.ToString(formFieldValues["LastActionByRole"]) : string.Empty);
                        break;
                    default:
        ...
        /// <param name="mailCustomValues">The mail custom values.</param>
        /// <param name="emailAttachments">The email attachments.</param>
        private void SendMail(ButtonActionStatus actionPerformed, ClientContext context, Web web, string currentUserID, int itemId, List<ApplicationStatus> approversDataFromList, string listname, int nextLevel, int currLevel, string role, Dictionary<string, string> paraml, Dictionary<string, string> mailCustomValues = null, List<FileDetails> emailAttachments = null)
        {
            string from = string.Empty, to = string.Empty, cc = string.Empty,
                //role = string.Empty,
            tmplName = string.Empty, strAllusers = string.Empty, nextApproverIds = string.Empty;
            ...
            FieldUserValue currentUser = new FieldUserValue();
            switch (actionPerformed)
            {
                case ButtonActionStatus.SaveAsDraft:
                    break;
                ...
                case ButtonActionStatus.SaveAndNoStatusUpdateWithEmail:
                    if (!string.IsNullOrEmpty(strAllusers) && approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        tmplName = EmailTemplateName.NEWREQUESTMAIL;
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.NEWREQUESTMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                case ButtonActionStatus.Save:
                    if (!string.IsNullOrEmpty(strAllusers) && approversDataFromList != null && approversDataFromList.Count != 0)
                ...
                case ButtonActionStatus.Delegate:
                    ...
                    {
                        from = currentUserID;
                        currentUser.LookupId = Convert.ToInt32(currentUserID);
                        // if (mailCustomValues != null) mailCustomValues[Parameter.CURRENTUSERALISE] = GetPersonValueFromPersonField(context, web, currentUser, Person.Alias);
                        ...
                        // role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == currLevel).Role;
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        tmplName = EmailTemplateName.DELEGATEAPPROVALMAIL;
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.DELEGATEAPPROVALMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                case ButtonActionStatus.NextApproval:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        to = allToUsers.Trim(',');
                        cc = approversDataFromList.Where(p => p.Role == UserRoles.CREATOR).First().Approver;
                        //var roles = approversDataFromList.Where(p => Convert.ToInt32(p.Levels) == currLevel).ToList().Select(m=> m.Role);
                        //string commasepratedRoles = string.Empty;
                        //foreach(var item in roles)
                        //{
                        //    commasepratedRoles +=  item + ",";
                        //}
                        //role = commasepratedRoles.Remove(commasepratedRoles.LastIndexOf(','));
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        tmplName = EmailTemplateName.APPROVALMAIL;
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.APPROVALMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                ...
                case ButtonActionStatus.Hold:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        tmplName = EmailTemplateName.HOLDMAIL;
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.HOLDMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                ...
                case ButtonActionStatus.Resume:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        to = approversDataFromList.FirstOrDefault(p => p.Role == UserRoles.CREATOR).Approver;
                        //  role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == nextLevel).Role;
                        tmplName = EmailTemplateName.RESUMEMAIL;
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.RESUMEMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                case ButtonActionStatus.SendBack:
                case ButtonActionStatus.BackToCreator:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.SENDBACKMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    //// creato to all viewer
                    break;
                case ButtonActionStatus.RestartToUpdate:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        //if (mailCustomValues != null)
                        //{
                        //    mailCustomValues[Parameter.CURRENTUSERALISE] = GetPersonValueFromPersonField(context, web, currentUser, Person.Alias);
                        //}
                        to = allToUsers.Trim(',');
                        cc = (cc.Trim(',') + "," + approversDataFromList.FirstOrDefault(p => p.Role == UserRoles.CREATOR).Approver).Trim(',');
                        //   role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == currLevel).Role;
                        tmplName = EmailTemplateName.RESTARTTOUPDATE;
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.RESTARTTOUPDATE, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                case ButtonActionStatus.Cancel:
                    if (!string.IsNullOrEmpty(strAllusers) && approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.REQUESTCANCELED, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                case ButtonActionStatus.Rejected:
                    if (!string.IsNullOrEmpty(strAllusers) && approversDataFromList != null && approversDataFromList.Count != 0)
                ...
                case ButtonActionStatus.Complete:
                    if (!string.IsNullOrEmpty(strAllusers) && approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        currentUser.LookupId = Convert.ToInt32(currentUserID);
                        // if (mailCustomValues != null) mailCustomValues[Parameter.CURRENTUSERALISE] = GetPersonValueFromPersonField(context, web, currentUser, Person.Alias);
                        to = approversDataFromList.FirstOrDefault(p => p.Role == UserRoles.CREATOR).Approver + "," + strAllusers.Trim(',');
                        //  cc = GetAllUsersFromGroup(context, web, "Admin");

                        // role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == currLevel).Role;
                        tmplName = EmailTemplateName.REQUESTCLOSERMAIL;
                        mailCustomValues[Parameter.MAILRECEIVERNAME] = GetReceiverName(GetNameUsingUserID(context, web, to));
                        email = eHelper.GetEmailBody(context, web, EmailTemplateName.REQUESTCLOSERMAIL, itemdetail, mailCustomValues, role, paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME]);
                    }
                    break;
                default:
                    break;
            ...
            if (email != null && email.Count == 2)
            {
                ...
                string ccUSer = GetEmailUsingUserID(context, web, cc);
                if (!string.IsNullOrWhiteSpace(ccUSer))
                {
                    cc = ccUSer + "," + GetAllUsersFromGroup(context, web, "Admin");
                }
                else
                {
                    cc = GetAllUsersFromGroup(context, web, "Admin");
                }
                eHelper.SendMail(paraml[Parameter.APPLICATIONNAME], paraml[Parameter.FORMNAME], tmplName, email["Subject"], email["Body"], from, to, cc, false, emailAttachments);
            }
        }
        ...
        public dynamic GetMultiplePersonField(ClientContext context, Web web, string emails, PropertyInfo property)
        {
            ...
            if (!string.IsNullOrEmpty(emails))
            {
                ...
                userValues = GetFieldUserValueFromUserIds(context, web, arrEmails);
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(applicationName) && !string.IsNullOrEmpty(formName) && !string.IsNullOrEmpty(userID) && form != null)
            {
                ...
                if (itemId > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(role))
                    {
                        ...
                        if (isSuperAdmin)
                        {
                            role = "LUM Item Code Creation Admin";
                        }
            ...
            foreach (ListItem item in items)
            {
                divisionAdminDetails.Add(
                    new DivisionAdminDetails()
                    {
                        ...
                        DivisionAdmins = GetUserIDsFromPersonField(context, web, item["DivisionAdmins"] as FieldUserValue[])
                    });
            }
            return divisionAdminDetails;
        }
                                            ...
                                            <And>
                                                ...
                                                <And>
                                                    ...
                                                    <Eq>
                                                        ...
                                                        <Value Type='UserMulti'>" + userName + @"</Value>
                                                    </Eq>
                                                </And>
                                            </And>
                    ...
                    if (!isHidden)
                    {
                        ...
                        else
                        {
                            ...
                            if (form.SectionsList[i].ListDetails[0].ListItemObject != null)
                            {
                                ...
                                // If Name require to display then enable this
                                ...
                                //    {
                                //        approvers = GetPersonValueFromPersonField(context, web, form.SectionsList[i].ListDetails[0].ListItemObject["NextApprover"] as FieldUserValue[],Person.Name);
                                //    }
                                ...
                                {
                                    string approvers = string.Empty;
                                    //if (form.SectionsList[i].ListDetails[0].ListItemObject["NextApproverAlias"] != null)
                                    if (form.SectionsList[i].ListDetails[0].ListItemObject["NextApprover"] != null)
                                    {
                        ...
                        if (isListColumn && isCurrentListCoumn)
                        {
                            ...
                            else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnID)
                            {
                                ...
                                if (users != null)
                                {
                                    ...
                                    {
                                        personEmails = GetPersonValueFromPersonField(context, web, users, Person.Name);
                                    }
                                    else
                                    {
                                        personEmails = GetUserIDsFromPersonField(context, web, users);
                                    }
                            ...
                            else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnName)
                            {
                                ...
                                if (users != null)
                                {
                                    ...
                                    userName = GetPersonValueFromPersonField(context, web, users, Person.Name);
                                    property.SetValue(actualSection, userName);
                                }
                            }
                            else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().ReturnAlias)
                            {
                                ...
                                if (users != null)
                                {
                                    var userAliases = string.Empty;
                                    userAliases = GetPersonValueFromPersonField(context, web, users, Person.Alias);
                                    property.SetValue(actualSection, userAliases);
                                }
                            }
                            else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson && property.GetCustomAttribute<IsPersonAttribute>().EmployeeTags)
                            {
                                ...
                                if (users != null)
                                {
                                    var empTags = GetEmployeeTagsFromPersonField(context, web, users);
                                    property.SetValue(actualSection, empTags);
                                }
                            }
                            else if (islookup)
                            {
                            ...
                            if (isListColumn && isCurrentListCoumn)
                            {
                                ...
                                else if (property.GetCustomAttribute<IsPersonAttribute>() != null && property.GetCustomAttribute<IsPersonAttribute>().IsPerson)
                                {
                                    ...
                                    if (users != null)
                                    {
                                        ...
                                        {
                                            user = GetPersonValueFromPersonField(context, web, users, Person.Name);
                                        }
                                        else if (property.GetCustomAttribute<IsPersonAttribute>().ReturnAlias)
                                        {
                                            user = GetPersonValueFromPersonField(context, web, users, Person.Alias);
                                        }
                                        else
                                        {
                                            user = GetUserIDsFromPersonField(context, web, users);
                                        }
                                        property.SetValue(actualSection, user);
                                    }
                                }
                                else if (islookup)
                                {
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(listName) && lookupId > 0)
            {
                ...
                foreach (ListItem item in items)
                {
                    try
                    {
                        ...
                        try
                        {
                            emailId = ((dynamic)(FieldUserValue)item["ActivityBy"]).Email;
                        }
                        catch
                        {
                            FieldUserValue user = (FieldUserValue)item["ActivityBy"];
                            if (user != null)
                            {
                                var varUser = EnsureUser(context, web, Convert.ToString(user.LookupId));
                                emailId = varUser.Email;
                    ...
                    {
                        Logger.Info("Error while Get Activity log for Listname: {0} - LookupId : {1} - {2} - {3}", listName, lookupId, e.StackTrace, e.Message);
                    }
                }
            }
            return activitySection;
                ...
                foreach (PropertyInfo property in properties)
                {
                    ...
                    else if (isCurrentApproverField)
                    {
                        ...
                        if (propertyValue != null)
                        {
                            ...
                            approverActivityLog += "~Approved/Updated date" + "\t" + DateTime.Now.ToString("dd/MM/yyyy");
                            approverActivityLog += "~Approved/Updated time" + "\t" + DateTime.Now.ToString("hh:mm tt");
                            approverActivityLog += "~" + "Approver Comment" + "\t" + approverDetails.Comments;
                            if (string.IsNullOrEmpty(strActivity))
                            {
        ...
        private UserDetails SetUserInformation(ClientContext context, Web web, ListItem item)
        {
            ...
            {
                user.ReportingManager = GetUserIDsFromPersonField(context, web, item["ReportingManager"] as FieldUserValue);
            }
            else
            {
