namespace BEL.DataAccessLayer
{
    ...
    /// </summary>
    /// <seealso cref="BEL.DataAccessLayer.BELDataAccessLayer" />
    public class MasterDataHelper : BELDataAccessLayer
    {
        ...
        public List<IMaster> GetMasterData(ClientContext context, Web web, List<IMaster> masters)
        {
            ...
            if (context != null && web != null && masters != null && masters.Count != 0)
            {
                ...
                {
                    IMaster master = null;
                    if (GetConfigVariable("UseRESTAPI").ToLower().Equals("true"))
                    {
        ...
        public List<UserDetails> GetAllEmployee(ClientContext context, Web web)
        {
            ...
            if (userInfoList == null)
            {
                ...
                if (userProfilesResult != null && userProfilesResult.Count != 0)
                {
                    ...
                    //GlobalCachingProvider.Instance.AddItem(web.ServerRelativeUrl + "/" + ListNames.APPROVERMASTERLIST, userInfoList);
                }
            }
            return userInfoList;
        }
        ...
        /// List of users
        /// </returns>
        private List<EmployeeDetails> GetAllEmployeeDetails(ClientContext context, Web web)
        {
            List<EmployeeDetails> empDetaillist = new List<EmployeeDetails>();
            string siteUrl = GetSiteURL(SiteURLs.ITEMCODECREATIONSITEURL); //this.GetConfigVariable(SiteURLs.ROOTSITEURL);
            List slaList = web.Lists.GetByTitle(ListNames.APPROVERMASTERLIST);
            ...
            foreach (ListItem item in items)
            {
                FieldUserValue fuv = (FieldUserValue)item["UserName"];
                empDetaillist.Add(new EmployeeDetails()
                {
                    UserID = Convert.ToString(fuv.LookupId),
                    UserName = fuv.LookupValue,
                    //Email = Convert.ToString(item["EmailAddress"]).ToLower(),
                    //Alias = Convert.ToString(item["Alias"]),
                    //EmployeeCode = Convert.ToString(item["EmployeeCode"]).ToLower(),
        ...
        /// <param name="web">The web.</param>
        /// <returns>
        /// List of Approvers Role filled by Current User Role for Create
        /// </returns>
        public List<EmployeeDetails> GetEmployeeDetailsList(ClientContext context, Web web)
        {
            var employeeDetailsesList =
                GlobalCachingProvider.Instance.GetItem(ListNames.APPROVERMASTERLIST, false) as
                    List<EmployeeDetails>;
            if (employeeDetailsesList == null)
            {
                employeeDetailsesList = this.GetAllEmployeeDetails(context, web);
                GlobalCachingProvider.Instance.AddItem(ListNames.APPROVERMASTERLIST, employeeDetailsesList);
            }
            return employeeDetailsesList;
        }

        /// <summary>
        /// Get Employee with Detail
        /// </summary>
        /// <param name="context">Context Object</param>
        /// <param name="web">Web Object</param>
        ...
        /// List of User Detail
        /// </returns>
        public EmployeeDetails GetEmployeeByEmail(ClientContext context, Web web, string email)
        {
            var emplDetailList = GlobalCachingProvider.Instance.GetItem(ListNames.EmployeeDetailMasterList, false) as List<EmployeeDetails> ??
                                                   this.GetEmployeeDetailsList(context, web);
            if (email != null)
                return emplDetailList.Find(details => details.Email.Equals(email.ToLower()));
            else
                return null;
        }

        /// <summary>
        /// Get Employee Aias Detail
        /// </summary>
        /// <param name="context">Context Object</param>
        /// <param name="web">Web Object</param>
        /// <param name="email">The email.</param>
        ...
        /// List of User Detail
        /// </returns>
        public string GetEmployeeAliasByEmail(ClientContext context, Web web, string email)
        {
            var employee = this.GetEmployeeByEmail(context, web, email);
            return employee == null ? string.Empty : employee.Alias;
        }
        ...
        ///// List of User Detail
        ///// </returns>
        //public List<EmployeeDetails> GetEmployeeAliasByTitle(ClientContext context, Web web, string title, string role = null)
        //{
        //    var emplDetailList = GlobalCachingProvider.Instance.GetItem(ListNames.EmployeeDetailMasterList, false) as List<EmployeeDetails> ??
        //                         this.GetEmployeeDetailsList(context, web);
        //    var employees = title != null
        //         ? emplDetailList.FindAll(details =>
        //                details.IsActive && ((!string.IsNullOrWhiteSpace(role) && details.Role.ToLower().Trim() == role.ToLower().Trim()) && (details.Alias.ToLower().Trim().Contains(title.ToLower().Trim()) || details.EmployeeCode.ToLower().Trim().Contains(title.ToLower().Trim()))))
        //         : null;
        //    return employees;
        //}
    }
