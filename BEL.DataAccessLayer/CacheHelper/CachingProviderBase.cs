namespace BEL.DataAccessLayer
{
    ...
    public abstract class CachingProviderBase : IDisposable
    {
        ...
        /// The log path
        /// </summary>
        private string logPath = Environment.GetEnvironmentVariable("TEMP");
        ...
        {
            File.Delete(string.Format("{0}\\CachingProvider_Errors.txt", this.logPath));
        }
        ...
        {
            using (TextWriter tw = File.AppendText(string.Format("{0}\\CachingProvider_Errors.txt", this.logPath)))
            {
                tw.WriteLine(text);              
            }
        }
